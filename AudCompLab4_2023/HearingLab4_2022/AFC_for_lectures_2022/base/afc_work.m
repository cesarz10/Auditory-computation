...
if ( def.soundEnable > 0 & ~def.skipDefaultSoundOutput )
    ...
    namescript2run = [def.expname '_user;'];
    if def.debug
        disp(namescript2run);
    end
   	eval(namescript2run); % calls user-function of the experiment
   	afc_ranpos;	% defines interval containing the test signal
   	afc_process;
end
...
if ( def.modelEnable > 0 )
    namescript2run = [work.vpname '_main'];
    if def.debug
        disp(namescript2run)
    end
	response = eval(namescript2run); % calling model and retrieve response (correct if response = work.position(end))
