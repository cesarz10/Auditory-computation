...
if isfield(mainDef,'normalJob')
    namescript2run = [expname '_cfg'];
    if def.debug
        disp(namescript2run);
    end
    eval(namescript2run); % overloades user defined values in structure def
end

if (exist([vpname '_cfg']) == 2)
    namescript2run = [vpname '_cfg'];
    if def.debug
        disp(namescript2run);
    end
    eval(namescript2run); % adds some vp parameters to structure def
end

if (exist([expname '_sim_cfg']) == 2)
    namescript2run = [expname '_sim_cfg'];
    if def.debug
        disp(namescript2run);
    end
    eval(namescript2run); % adds some simulation parameters to structure def
end
...
if ~isfield(def,'version')
    def.version = '1.40 build 1';[def.measurementProcedure '_cfg']
end
...
if isfield(def,'expcond') % experimental condition
    def.expparnum = def.expparnum + 1;
...
% fix path bug (solution only valid for windows-based), comment by Alejandro
if ~isempty( def.result_path )
    if max( findstr( def.result_path, filesep)) ~= length( def.result_path )
        def.result_path = [def.result_path filesep];
    end
end
if ~isempty( def.control_path )
    if max( findstr( def.control_path, filesep)) ~= length( def.control_path )
        def.control_path = [def.control_path filesep];
    end
end

if ~isempty(def.stateLogPath )
    if max( findstr( def.stateLogPath, filesep)) ~= length( def.stateLogPath )
        def.stateLogPath = [def.stateLogPath filesep];
    end
end
% end fix bug
...
work = struct( ...		 % initialise structure work
...
% initilise or read control file
...
while ( work.numrun <= work.numrunMax & abortall == 0)	% changed 4/7/2000 checks whether experiment is already finished
    ...
    elseif ( strcmp(def.measurementProcedure, 'transformedUpDown') )
        for (i=1:def.interleavenum)
            try
                ...
                work.expvarnext{i} = def.startvar;
            end
        end
    ...
    work.answer_extra1{1} = [];						% vector of user responses (extra 1), added by AO
    work.answer_extra2{1} = [];						% vector of user responses (extra 2), added by AO
    work.answer_extra3{1} = [];						% vector of user responses (extra 3), added by AO
    ...
    namescript2run = [def.expname '_set'];
    disp(namescript2run);
    eval(namescript2run);	% calls set-function of the experiment
    ...
    if ( def.modelEnable > 0 )
        ...
        namescript2run = [work.vpname '_init'];
        disp(namescript2run);
        eval(namescript2run);	% calls init-function of the model
    end
