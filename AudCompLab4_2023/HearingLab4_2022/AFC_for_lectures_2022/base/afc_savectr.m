...
switch sel				% 0 = checks whether control file is existing, if not it is generated; 1 = write only 
    case 0
        ...
        warning('Control file found...is this what you want?')
        % read control file (already existing and already opened in 'r' mode)
        %fclose(e);
        ...
        try
            % This should always work with newer versions of MATLAB(by AO)
            fileID = fopen(str);       
            tmp = textscan(fileID,'%s','Headerlines',headerlines);
            tmp = tmp{1};
            fclose(fileID);
        catch
            warning('Code as in older versions of MATLAB');
        ...
        tmprownum=size(tmp,1)-1; % added by AO
