...
global simdef  % added by AO on 5/12/2018
    ...
    if def.modelEnable
        ...
        if isfield(simwork,'extra_param')
            ...
            for ii = 1:length(simwork.extra_param)
                eval(sprintf('extra{%.0f} = %s;',ii,simwork.extra_param{ii})); % simwork.mueact_nonoise
            end
            
            % Assignment for ii = 1 outside the for loop, only for clarity
            work.answer_extra1{work.pvind} = [work.answer_extra1{work.pvind} extra{1}]; 
            for ii = 2:length(simwork.extra_param)
                try
                    eval(sprintf('work.answer_extra%.0f{work.pvind} = [work.answer_extra%.0f{work.pvind} extra{%.0f}];',ii,ii,ii));
                catch
                    eval(sprintf('work.answer_extra%.0f{work.pvind} = [extra{%.0f}];',ii,ii));
                end
            end
        end
    end
