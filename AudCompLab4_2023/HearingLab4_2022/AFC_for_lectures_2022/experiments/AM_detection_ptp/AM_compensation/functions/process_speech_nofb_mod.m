function [stim,env,g,minima,envp,env_mod]=process_speech_nofb_mod(stim,sp_par,spl70_flag,seg,height,prominence,distance)
...
if any(sp_par~=[13,3,3])
    ...
    xdata_min=zeros(size(stim,1));
    xdata_max=zeros(size(stim,1));
    gf=zeros(size(stim,1));
    gl=zeros(size(stim,1));
    env=zeros(size(stim));
    envp=zeros(size(stim));
    env_mod=zeros(size(stim));
    g=zeros(size(stim));
    ...
    for j=1:length(minima)-1
        ...
        for n=1:size(stim_seg,1)
            % temporary fix
            if length(minima)==2
                xdata_min(n)=min(stimpo(n,100:end-100));
            else
                xdata_min(n)=min(stimpo(n,index));
            end
            xdata_max(n)=max(stimpo(n,index)-xdata_min(n));
            
            stimp(n,index)=(stimpo(n,index)-xdata_min(n))./xdata_max(n);
            stimp(n,stimp(n,index)<0)=0;
            
            env(n,index)=stimp(n,index).*xdata(n)./max(abs(stimp(n,index)));
            env_mod(n,index)=stimpo(n,index).*xdata(n)./max(abs(stimpo(n,index)));
            
            gl(n)=1./max(1./(1+exp(scale.*(-b70.*env(n,index)./gf(n)+c(n)))));
            g(n,index)=gl(n)./(1+exp(scale.*(-b70.*env(n,index)./gf(n)+c(n))));
        
            envp(n,index)=g(n,index).*env(n,index).*xdata_max(n)./xdata(n)+xdata_min(n);
            envp(n,index)=envp(n,index).*xdata(n)./max(abs(envp(n,index)));
            g(n,index)=envp(n,index)./env_mod(n,index);
        end
    end
