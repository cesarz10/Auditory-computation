function [stim,env,g]=process_speech_nofb_mod_cleanenv(stim,stim_noisy,sp_par,spl70_flag,seg,height,prominence,distance)
...
if any(sp_par~=[13,3,3])
    ...
    for j=1:length(minima)-1
        ...
        for n=1:size(stim_seg,1)            
            ...
            env_mod(n,index)=g(n,index).*env(n,index).*xdata_max(n)./xdata(n)+xdata_min(n);
            g(n,index)=env_mod(n,index)./stimpo(n,index);
        end
    end
    stim=g.*stim_noisy;
end
