...
%%%% for extracting the outputs (keep commented)
% tuser = setup.modsine .* (1 + (setup.modcosine)); % fully-modulated for storing
% % tuser = tuser * (max(abs(tref1)) / max(abs(tuser))); % same peak
% tuser = tuser * scaling_factor; % same carrier amplitude
% cs_par=[fix(work.exppar2./100),mod(fix(work.exppar2./10),10),mod(work.exppar2,10)];
% tuserp = process_speech_nofb_mod(tuser,cs_par,1); %process_am_tone_nofb(tuser,cs_par,1);
% max(abs(tuser))
% max(abs(tuserp))
